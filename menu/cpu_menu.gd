...
#func new_task():
	...
	#var random_task = Global.tasks[randi() % Global.tasks.size()] # valib suvalise ülesande listist Global.tasks = ["locate-SSD-info","deliver-SSD-info"]
	#$Task_Panel/ScrollContainer/VBoxContainer.add_child(task_block)
	#if random_task == "locate-SSD-info" or random_task == "deliver-SSD-info" and psu_fix_cooldown == true:
...
#
	...
	#elif random_task == "deliver-SSD-info":
