...
func _process(delta):
	...
	if Global.task_ongoing != "":
		...
		CPU_subtasks = ["Transpordi '" + deliver_cpu_item_file + " info' CPU-majja.", "Mine rongi peale ja käivita see!", "Sisene CPU-majja.", "Aseta kasti sees olev '" + deliver_cpu_item_file + " info' liikuvale lindile.", "Võta skännerist väljastatud kümnendarvuline kood ja\nteisenda see kahendarvuks, kasutades CPU-majas asuvat arvutit."]
